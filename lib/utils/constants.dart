...
class AppConstants {
  ...
  /// Chunk size used by the streaming RFCOMM file transfer.
  ///
  /// Files are read/written in chunks of this size so that arbitrarily large
  /// files (GBs) never need to be loaded into memory. 16 KB is a good balance
  /// between throughput and per-chunk overhead.
  static const int streamingChunkSizeBytes = 16384;
  
  /// Number of unacknowledged chunks the sender keeps in flight.
  static const int fileTransferWindowSize = 4;
  
  /// Time after which an unacknowledged chunk is retransmitted.
  static const int fileChunkAckTimeoutMs = 8000;
  
  /// Max retransmissions of a single chunk before the transfer fails.
  static const int fileChunkMaxRetries = 5;
  
  /// Frame format safety limit (a single framed packet may never exceed this).
  static const int maxFrameSizeBytes = 32 * 1024 * 1024;
...
// The nested "types" classes below are declared at top level because Dart
// does not allow classes to be declared inside other classes.
