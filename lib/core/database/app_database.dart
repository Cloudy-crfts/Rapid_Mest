...
class AppDatabase {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 2) {
      // v2: messages got a wire-level `message_id` column so delivery acks can
      // be mapped back to the right row.
  ...
  /// Read a single setting value, or null if it does not exist.
  Future<String?> getSetting(String key) async {
    try {
      ...
      AppLogger.error('Failed to read setting: $key', 'Database', e);
      return null;
    }
  }

  /// Write (insert or replace) a single setting value.
  Future<void> setSetting(String key, String value) async {
    try {
      ...
      AppLogger.error('Failed to write setting: $key', 'Database', e);
    }
  }
