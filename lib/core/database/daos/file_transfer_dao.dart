...
class FileTransferDao {
  ...
  Future<List<FileTransfer>> getPendingTransfers() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      'file_transfers',
      where: 'status = ?',
      whereArgs: [TransferStatus.pending],
      orderBy: 'created_at ASC',
    );
  ...
  Future<List<FileTransfer>> getResumableTransfers() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      'file_transfers',
      where: '''status = ?''',
      whereArgs: [TransferStatus.paused],
  ...
  Future<void> recordChunkReceived({
    ...
    required Set<int> allReceivedChunks,
    required int totalBytesSoFar,
  }) async {
    final db = await _db;
  ...
  Future<void> acceptTransfer(int transferId) async {
    await updateStatus(transferId, TransferStatus.accepted);
  }

  /// Reject a pending transfer
  Future<void> rejectTransfer(int transferId) async {
    await updateStatus(transferId, TransferStatus.rejected);
  }

  /// Pause a transfer
  Future<void> pauseTransfer(int transferId) async {
    await updateStatus(transferId, TransferStatus.paused);
  }

  /// Resume a paused transfer
  Future<void> resumeTransfer(int transferId) async {
    await updateStatus(transferId, TransferStatus.transferring);
  }
  ...
  Future<void> completeTransfer(int transferId, {String? actualChecksum}) async {
    ...
    // bytes_transferred is set to the full file size by referencing the
    // file_size column (a raw UPDATE so we can use a column expression).
    await db.rawUpdate(
  ...
  Future<void> failTransfer(int transferId, String reason) async {
    await updateStatus(transferId, TransferStatus.failed, failureReason: reason);
