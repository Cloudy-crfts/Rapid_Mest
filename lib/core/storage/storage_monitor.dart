...
class StorageMonitor {
  ...
  // Thresholds (in bytes)
  late int _criticalThreshold;    // Below this = cannot operate
  late int _warningThreshold;     // Below this = show warnings
  late int _recommendedMinimum;   // Recommended minimum for smooth operation
  ...
  /// Get detailed storage information using the platform (StatFs on Android).
  Future<StorageInfo> _getDetailedStorageInfo(String path) async {
    try {
      // The native layer reports the real free/total space of the volume the
      // app's files live on (android.os.StatFs).
      const channel = MethodChannel('com.rapidmesh/native');
      final result =
          await channel.invokeMapMethod<String, dynamic>('getStorageInfo');

      final total = (result?['totalSpace'] as num?)?.toInt() ?? 0;
      final free = (result?['freeSpace'] as num?)?.toInt() ?? 0;
      final used = (result?['usedSpace'] as num?)?.toInt() ?? 0;
      ...
      // Fall back to a permissive estimate so transfers are not blocked when
      // the native stat is unavailable (older devices / non-Android builds).
      return _fallbackEstimate(path);
    } catch (e) {
  ...
  StorageInfo _fallbackEstimate(String path) {
