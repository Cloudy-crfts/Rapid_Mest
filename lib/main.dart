...
void main() async {
  ...
  // Load the user's profile (display name + avatar) before the handshake
  // needs it.
...
void _setupConnectionRequestListener() {
  ...
  bt.events.listen((event) {
    ...
    } else if (event is FileTransferRequestEvent) {
      // Another device wants to send us a file - ask before accepting.
      FilePermissionDialog.show(
        context: ctx,
        senderName: bt.getConnectionInfo(event.address)?.remoteName ?? 'Device',
        fileName: event.fileName,
        fileSize: event.fileSize,
        mimeType: event.mimeType,
      ).then((accepted) {
        if (accepted == true) {
          bt.fileTransfers.acceptIncoming(event.transferId);
        } else {
          bt.fileTransfers.rejectIncoming(event.transferId);
        }
      });
    } else if (event is FileTransferCompletedEvent && !event.outgoing) {
      // Show a quick toast when a file has finished arriving.
      final msg = event.success
          ? '✓ Received ${event.fileName}'
          : '✗ ${event.fileName}: ${event.error ?? 'failed'}';
