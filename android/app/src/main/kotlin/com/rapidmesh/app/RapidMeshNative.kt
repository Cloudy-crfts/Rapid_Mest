...
class RapidMeshNative(private val context: Context, private val engine: FlutterEngine) {
    ...
    /** Reports real free/total space of the volume the app's files live on. */
    private fun getStorageInfo(): Map<String, Any> {
        return try {
            ...
            mapOf(
                "totalSpace" to total,
                "freeSpace" to available,
                "usedSpace" to (total - available)
            )
        } catch (e: Exception) {
            mapOf("totalSpace" to 0L, "freeSpace" to 0L, "usedSpace" to 0L)
        }
    }
